 ...
 * 
 */
public class ZSaver implements Saver {
	public void save(GameController inGame, WorldList base, String file)
			throws IOException {
		if (!(inGame instanceof ZGameController)) {
			throw new IllegalArgumentException(
					"This class only supports ZGameControllers");
		}

		ZGameController game = (ZGameController) inGame;
		...
		output.writeSignedLEShort(torches);
		int tcycles = game.getState().tcycles;
		output.writeSignedLEShort(tcycles);
		...
		output.writeZeroPadding((7 + (15 * 16)) - 20);
		output.writeZZTString("FREEZE   ", 9);
	...
	public void saveBoard(Board b, UnsignedDataOutputStream out)
		...
		if (b == null) { // board was not loaded correctly in the first place
			b = new DefaultZWorldCreator().createDefaultBoard();
			// TODO add a message
		}
		...
		obuf.writeZZTString(b.getState().boardName, 33);
		obuf.writeZeroPadding(17);

		saveRLEBoardData(b, obuf);

		obuf.write(b.getState().shots);
		obuf.write(b.getState().dark);
		...
		obuf.writeZZTString(b.getState().message, 58);
	...
	private void saveRLEBoardData(Board b, UnsignedDataOutputStream obuf)
		...
		int x = 0, y = 0;
		for (int pos = 0; pos < ZBoard.BOARD_WIDTH * ZBoard.BOARD_HEIGHT; pos++) {
