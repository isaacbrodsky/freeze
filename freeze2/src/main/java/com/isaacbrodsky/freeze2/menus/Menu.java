 ...
 * 
 */
public class Menu<T> extends MenuUtils implements UIInteraction {
	...
	public Menu(String title, String data, MenuCallback<T> callback) {
	...
	public Menu(String title, String data, MenuCallback<T> callback,
			SendMode sendLast, boolean cancellable, T rider) {
		...
		for (int i = topPos; i < PAGE_HEIGHT + topPos; i++) {
			...
			case SELECTABLE:
				renderer.renderText(3 + offsetX, height + offsetY, 16,
						ElementColoring.forNames(NamedColor.PURPLE, NamedColor.DARKBLUE));
