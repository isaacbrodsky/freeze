 ...
 * 
 */
public class TextInput<T> extends MenuUtils implements UIInteraction,
		TypingInteraction {
	...
	public TextInput(String title, String data, MenuCallback<T> callback,
			boolean cancellable, T rider) {
		this(title, data, callback, cancellable, rider,
				Renderer.DISPLAY_HEIGHT - 2);
	}

	public TextInput(String title, String data, MenuCallback<T> callback,
			boolean cancellable, T rider, int y) {
